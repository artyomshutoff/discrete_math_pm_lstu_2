...
def сarnot(expr, variables):
	...
	def minimization(type_, map_, left, right, vars_left, vars_right):
		...
		while p > -1:
			...
			for i in combo(A, r = area):
				...
				if not isRect(i, map_):
					continue
				check = all([map_[int(j[0])][int(j[1])] for j in i])
				if type_ != 'dnf':
					check = all([not i for i in [map_[int(j[0])][int(j[1])] for j in i]])
				if check:
		...
		return f' {connector} '.join(map(lambda x: f'({x})',minimized))
