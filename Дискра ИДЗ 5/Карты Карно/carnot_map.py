...
def carnot(expr, variables):
	...
	check_for_const = [i[-1] for i in A]
	...
	def minimization(type_, map_, left, right, vars_left, vars_right):
		...
		def area_accessibility(using_cells, points, type_):
			for i in points:
				if using_cells[int(i[0])][int(i[1])] == '*':
					return 1
			return 0
		...
		using_cells = []
		for i in range(len(map_)):
			using_cells.append(['*' for j in range(len(map_[i]))])
	...
	if len(set(check_for_const)) > 1:
		...
		if all(check_for_const):
			...
			print('\nСКНФ не существует.')
		elif all([not i for i in check_for_const]):
			print('\nСДНФ не существует.')
